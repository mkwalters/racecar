...
class GalileoCourseSelectionScene: CourseSelectionScene {
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if easyButton.contains(touchLocation) {
            ...
            let gameScene = GalileoCourseOne(type: "time_trials", previous_move_count: 0, number_of_lives: 1)
        ...
        if course_two.contains(touchLocation){
            let reveal = SKTransition.doorsOpenHorizontal(withDuration: 0.25)
            let menuScene = GalileoCourseTwo(type: "time_trials", previous_move_count: 0, number_of_lives: 1)
            self.view?.presentScene(menuScene, transition: reveal)
        }
        
        if course_three.contains(touchLocation){
            let reveal = SKTransition.doorsOpenHorizontal(withDuration: 0.25)
            let menuScene = GalileoCourseThree(type: "time_trials", previous_move_count: 0, number_of_lives: 1)
            self.view?.presentScene(menuScene, transition: reveal)
        }
        
        if course_four.contains(touchLocation){
            let reveal = SKTransition.doorsOpenHorizontal(withDuration: 0.25)
            let menuScene = GalileoCourseFour(type: "time_trials", previous_move_count: 0, number_of_lives: 1)
            self.view?.presentScene(menuScene, transition: reveal)
        }
